...
// Discord Limits
const limits = { title: 256, body: 2000 };

export default function DiscordDesignPanelPage() {
    const navigate = useNavigate();
    const location = useLocation();

    // State initialization from location state (for new or existing templates)
    const [channelId] = useState<string>('discord');
    ...
    const channelInfo = getChannelMetadata('discord');
        ...
        <div className="flex flex-col h-screen overflow-hidden bg-primary text-primary">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col bg-[#050505] overflow-hidden relative">
                    ...
                    <div className="flex-1 flex items-center justify-center p-8 bg-[#1e1f22]">
                        {/* Discord Desktop Mockup */}
                        <div className="relative w-full max-w-2xl h-[500px]">
                            <div className="absolute inset-0 bg-[#313338] rounded-xl border border-white/5 shadow-2xl overflow-hidden flex flex-col font-sans">
                                {/* Discord Header */}
                                <div className="h-12 bg-[#2b2d31] flex items-center px-4 gap-3 border-b border-white/5 shadow-sm shrink-0">
                                    <span className="text-[#80848e] text-xl font-bold">#</span>
                                    <span className="text-white font-semibold flex items-center gap-2">notifications</span>
                                </div>
                                {/* Discord Body */}
                                    ...
                                    <div className="flex gap-4 group">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-baseline gap-2 mb-1">
                                                <span className="text-white font-medium hover:underline cursor-pointer">Fertile Notify</span>
                                                <span className="bg-[#5865F2] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-[3px] flex items-center gap-1 leading-none h-4">
                                                    APP
                                                    <svg className="w-3 h-3" viewBox="0 0 16 16" fill="currentColor"><path fillRule="evenodd" clipRule="evenodd" d="M12.426 3.654L14.773 6l-2.347 2.346v3.313h-3.313L6.766 14 4.42 11.654H1.106V8.34L-1.24 6l2.346-2.346V.34h3.313L6.766-2 9.113.34h3.313v3.314zM8.34 9C8.34 9 9 9 9.34 8.24l3.16-5.8c-.34-.4-.84-.5-1.34-.4l-2.82 4L6.18 5c-.44-.24-.96-.1-1.26.26l-2.06 2.6c-.22.28-.2.66.04.92.54.58 4.02 1.62 5.44.22z"></path></svg>
                                                </span>
                                                <span className="text-[#949ba4] text-xs">Today at {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                            </div>
                                            {/* Embed Message */}
                                            <div className="mt-1 bg-[#2b2d31] rounded-[4px] border-l-4 border-[#5865F2] p-4 max-w-xl">
                                                {title && (
                                                    <div className="text-white font-semibold text-[15px] mb-2 leading-snug break-words">
                                                        {title}
                                                    </div>
                                                )}
                                                <div className="text-[#dbdee1] text-[14px] leading-relaxed whitespace-pre-wrap break-words">
                                                    {message || 'Your Discord embedded message will appear here...'}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="px-4 pb-4 shrink-0">
                                    <div className="bg-[#383a40] rounded-lg h-11 flex items-center px-4">
                                        <span className="text-[#80848e] text-[15px]">Message #notifications</span>
                                    </div>
                                </div>
                            </div>
                        </div>
