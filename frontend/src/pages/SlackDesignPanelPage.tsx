...
export default function SlackDesignPanelPage() {
    const navigate = useNavigate();
    const location = useLocation();

    // State initialization from location state (for new or existing templates)
    const [channelId] = useState<string>('slack');
    ...
    const channelInfo = getChannelMetadata('slack');
                ...
                <div className="flex-1 flex flex-col bg-[#050505] overflow-hidden relative">
                    ...
                    <div className="flex-1 flex items-center justify-center p-8 bg-[#f8f9fa]">
                        ...
                        <div className="relative w-full max-w-2xl h-[500px]">
                            <div className="absolute inset-0 bg-white rounded-xl border border-gray-200 shadow-2xl overflow-hidden flex flex-col font-sans">
                                {/* Slack Header */}
                                <div className="h-14 border-b border-gray-200 flex items-center px-5 gap-2 shrink-0 bg-white z-10 shadow-sm">
                                    <span className="text-gray-900 font-black flex items-center gap-1.5 text-base">
                                        <svg className="w-4 h-4 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="4" y1="9" x2="20" y2="9"></line><line x1="4" y1="15" x2="20" y2="15"></line><line x1="10" y1="3" x2="8" y2="21"></line><line x1="16" y1="3" x2="14" y2="21"></line></svg>
                                        notifications
                                        <svg className="w-3.5 h-3.5 text-gray-400 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                                    </span>
                                </div>
                                {/* Slack Body */}
                                <div className="py-4 px-5 overflow-y-auto flex-1">
                                    <div className="flex gap-3 group">
                                        <div className="w-10 h-10 rounded-[4px] bg-[#4a154b] flex items-center justify-center shrink-0 overflow-hidden shadow-sm mt-0.5">
                                            <span className="text-white text-xl">{channelInfo.icon}</span>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-baseline gap-2 mb-1">
                                                <span className="text-gray-900 font-bold hover:underline cursor-pointer text-[15px]">Fertile Notify</span>
                                                <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-1.5 rounded flex items-center h-4 uppercase tracking-wider">APP</span>
                                                <span className="text-gray-500 text-xs ml-0.5">{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                            </div>
                                            {/* Slack Attachment */}
                                            <div className="mt-1.5 pl-3 border-l-4 border-[#e3e4e6] group-hover:border-gray-300 transition-colors">
                                                {title && (
                                                    <div className="text-[#0b4c8c] font-bold text-[15px] mb-1.5 leading-snug break-words">
                                                        {title}
                                                    </div>
                                                )}
                                                <div className="text-gray-800 text-[15px] leading-relaxed whitespace-pre-wrap break-words">
                                                    {message || 'Your Slack message attachment will appear here...'}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="px-5 pb-5 shrink-0">
                                    <div className="border border-gray-400 rounded-md h-11 flex items-center px-3 shadow-sm bg-white">
                                        <span className="text-gray-500 text-[15px]">Message #notifications</span>
                                    </div>
                                </div>
                            </div>
                        </div>
