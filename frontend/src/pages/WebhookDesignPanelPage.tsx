...
export default function WebhookDesignPanelPage() {
    const navigate = useNavigate();
    const location = useLocation();

    // State initialization from location state (for new or existing templates)
    const [channelId] = useState<string>('webhook');
    ...
    const channelInfo = getChannelMetadata('webhook');
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col bg-[#050505] overflow-hidden relative">
                    ...
                    <div className="flex-1 flex items-center justify-center p-8 bg-[#0d1117]">
                        {/* Webhook JSON Payload Mockup */}
                        <div className="relative w-full max-w-2xl h-[400px]">
                            <div className="absolute inset-0 bg-[#161b22] rounded-xl border border-gray-700 shadow-2xl overflow-hidden flex flex-col font-mono text-sm">
                                {/* Header */}
                                <div className="h-10 border-b border-gray-700 flex items-center px-4 bg-[#0d1117] shrink-0 justify-between">
                                    <div className="flex items-center gap-2">
                                        <div className="w-3 h-3 rounded-full bg-red-500/80"></div>
                                        <div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
                                        <div className="w-3 h-3 rounded-full bg-green-500/80"></div>
                                    </div>
                                    <span className="text-gray-400 text-xs tracking-wider">POST /api/webhook</span>
                                </div>
                                {/* Body */}
                                <div className="p-6 overflow-y-auto flex-1 text-gray-300">
                                    <pre className="whitespace-pre-wrap word-break">
                                        <span className="text-blue-400">&#123;</span>
                                        <span className="text-blue-300">"event"</span>: <span className="text-green-300">"notification.triggered"</span>,
                                        <span className="text-blue-300">"timestamp"</span>: <span className="text-green-300">"{new Date().toISOString()}"</span>,
                                        <span className="text-blue-300">"payload"</span>: <span className="text-blue-400">&#123;</span>
                                        <span className="text-blue-300">"title"</span>: <span className="text-green-300">"{title || ''}"</span>,
                                        <span className="text-blue-300">"message"</span>: <span className="text-green-300">"{message ? message.replace(/\n/g, '\\n') : ''}"</span>,
                                        <span className="text-blue-300">"channel"</span>: <span className="text-green-300">"webhook"</span>
                                        <span className="text-blue-400">&#125;</span>
                                        <span className="text-blue-400">&#125;</span>
                                    </pre>
                                </div>
